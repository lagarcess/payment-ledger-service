...
- **Where the code lives:** `sandbox_ledger.py` handles the core math, balancing logic, and global invariant checks.
...
- **Where the code lives:** `sandbox_ledger.py` performs all ledger math strictly using Python integers. `script.js` reformats the integer back into a localized currency string.
...
- **Where the code lives:** Look at the `_seed_data()` function in `sandbox_ledger.py`, which meticulously structures the initial capital injection journal entries.
...
- **Where the code lives:** `sandbox_ledger.py` dynamically calculates the exchange rate and creates the intermediate clearing legs during transaction processing. `static/script.js` handles the real-time diagram rendering.
...
- **Where the code lives:** `sandbox_ledger.py` wraps all entry creations inside a strict `with session.begin():` block that rolls back upon any exception.
...
- **Where the code lives:** `sandbox_ledger.py` enforces a unique database constraint on the key. `api.py` catches duplicate errors and returns a 409 status code. 
...
- **Where the code lives:** `sandbox_ledger.py` strictly checks account balances before creating any ledger entries and raises an `InsufficientFundsError` if the transaction would result in a negative balance.
...
- **Where the code lives:** `api.py` exposes the `/api/reset` endpoint, which drops the SQLite tables and triggers the seed function in `sandbox_ledger.py`.

## Tech Stack

- **Backend:** Python 3.10+, FastAPI, SQLAlchemy, SQLite (`api.py`, `sandbox_ledger.py`)
